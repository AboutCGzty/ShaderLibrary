Shader "ZTY/ImageEffect/Blur/Average/9Tap"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float4 s1 = 0.0;
                // 正中间0点坐标
                s1 += tex2D(_MainTex, screen_uv); // [0,0]
                // 四个角的坐标
                ...
                // 上下左右坐标
                s1 += tex2D(_MainTex, screen_uv + _MainTex_TexelSize.xy * float2(0.0, blur_pos.x)); // [0,1]
                s1 += tex2D(_MainTex, screen_uv + _MainTex_TexelSize.xy * float2(blur_pos.z, 0.0)); // [1,0]
                s1 += tex2D(_MainTex, screen_uv + _MainTex_TexelSize.xy * float2(0.0, blur_pos.y)); // [0,-1]
                s1 += tex2D(_MainTex, screen_uv + _MainTex_TexelSize.xy * float2(blur_pos.w, 0.0)); // [-1,0]
